...
exports.saveProduct = (request, response, next) => {
    ...
    console.log(request.body.productQuantity)
    ...
    if (file.name) {
        ...
        file.mv(filePath, err => {
            if (!err) {
                if (request.body.productName && request.body.productPrice && request.body.productQuantity && request.body.categoryId && fileName && request.body.description) {
                    let product = new Product(request.body.productName, request.body.productPrice, request.body.productQuantity, request.body.categoryId, fileName, request.body.description);
